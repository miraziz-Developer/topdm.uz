...
export function HomeDealsRow({ variant, products, loading }: Props) {
  ...
  return (
    <section className={`mx-auto max-w-7xl px-4 py-3 sm:px-6 sm:py-4 bg-gradient-to-r ${cfg.bg}`}>
      ...
      <div className="scrollbar-hide -mx-1 flex gap-3 overflow-x-auto px-1 pb-1 snap-x snap-mandatory">
        {loading
          ...
          : products.length === 0
            ...
            : products.map((p) => <DealProductCard key={p.id} product={p} variant={variant} />)}
      </div>
    </section>
  );
}
